...
import { alertsBySeverityData } from '../../data/alertsData';
        ...
        <div className="relative w-28 h-28 sm:w-32 sm:h-32 flex-shrink-0 flex items-center justify-center">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center select-none pointer-events-none">
            <span className="text-base sm:text-lg font-black text-slate-900 leading-tight">
              {alertsBySeverityData.total}
            </span>
            <span className="text-[8px] font-bold text-slate-400">Total Alerts</span>
          </div>
        </div>

        {/* Legend List */}
        <div className="flex flex-col space-y-2 min-w-0 flex-1">
          {alertsBySeverityData.breakdown.map((item) => (
