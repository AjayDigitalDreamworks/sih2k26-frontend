...
import { fieldReportsKPIs } from '../../data/fieldReportsData';

export default function FieldReportsKPIs() {
  const getIcon = (iconName) => {
    switch (iconName) {
      case 'file-text':
        return <FileText className="w-5 h-5 text-emerald-600 stroke-[2.2]" />;
      case 'clipboard-check':
        return <ClipboardCheck className="w-5 h-5 text-blue-600 stroke-[2.2]" />;
      case 'check-circle':
        return <CheckCircle2 className="w-5 h-5 text-purple-600 stroke-[2.2]" />;
      case 'clock':
        return <Clock className="w-5 h-5 text-orange-500 stroke-[2.2]" />;
      case 'alert-circle':
        return <AlertCircle className="w-5 h-5 text-rose-600 stroke-[2.2]" />;
      default:
        return <FileText className="w-5 h-5 text-emerald-600 stroke-[2.2]" />;
    }
  };
